...
@Getter@Setter
public class BookingsByTypeView implements RpcView<BookingsByTypeView, BookingsByTypeRow> {

    @Ignored
    private transient List<BookingsByTypeRow> l;


    @Override
    public List<BookingsByTypeRow> rpc(BookingsByTypeView filters, int offset, int limit) throws Throwable {
        List<BookingsByTypeRow> l = null;
        if (l == null) l = fetch();
        else l = consume();
        return l;
    }

    private List<BookingsByTypeRow> fetch() throws Throwable {
        List<BookingsByTypeRow> l = new ArrayList<>();
        Helper.transact(em -> {
    ...
    @Override
    public int gatherCount(BookingsByTypeView filters) throws Throwable {
        List<BookingsByTypeRow> l = null;
        if (l == null) l = fetch();
        else l = consume();
        return l.size();
    ...
    private List<BookingsByTypeRow> consume() {
        List<BookingsByTypeRow> aux = l;
        l = null;
        return aux;
    }

    private Query createQuery(EntityManager em) {
        Query q = em.createQuery("select new " + BookingsByTypeRow.class.getName() + "(s.start, s.office, count(s), min(s.effectiveProcessingStatus)) from " + FreeTextService.class.getName() + " s where s.start >= :d group by s.start, s.office order by s.office.name, s.start").setParameter("d", LocalDate.now());
        return q;
    }
    ...
    @Override
    public void decorateGrid(Grid<BookingsByTypeRow> grid) {
        grid.getColumn("bookings").setStyleGenerator(new StyleGenerator<BookingsByTypeRow>() {
            @Override
            public String apply(BookingsByTypeRow s) {
                String css = "";
