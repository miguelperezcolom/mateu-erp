...
@Getter@Setter
public class FlightsView implements RpcView<FlightsView, FlightRow> {
    ...
    @Ignored
    private transient List<FlightRow> l;

    @Action
    public static void changeFlight(List<FlightRow> selection, String FlightNumber, int flightTime) {

    }


    @Override
    public List<FlightRow> rpc(FlightsView filters, int offset, int limit) throws Throwable {
        List<FlightRow> l = null;
        if (l == null) l = fetch();
        else l = consume();
        return l;
    }

    private List<FlightRow> fetch() throws Throwable {
        List<FlightRow> l = new ArrayList<>();
        Helper.transact(em -> {
    ...
    @Override
    public int gatherCount(FlightsView filters) throws Throwable {
        List<FlightRow> l = null;
        if (l == null) l = fetch();
        else l = consume();
        return l.size();
    ...
    private List<FlightRow> consume() {
        List<FlightRow> aux = l;
        l = null;
        return aux;
    }
    ...
    @Override
    public void decorateGrid(Grid<FlightRow> grid) {
        grid.getColumn("bookings").setStyleGenerator(new StyleGenerator<FlightRow>() {
            @Override
            public String apply(FlightRow s) {
                String css = "";
